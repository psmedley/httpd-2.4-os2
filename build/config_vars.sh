...
if test -f ${DESTDIR}/extras/bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/extras/bin/apr-1-config
   APU_CONFIG=${DESTDIR}/extras/bin/apu-1-config
else
...
test -n "/extras/bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/apache24/build"
